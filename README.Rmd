...
```{r, eval = FALSE}
devtools::install_github("SwissClinicalTrialOrganisation/shiny_secuTrialR")
```
...
```{r, eval = FALSE}
# run tests
devtools::test("shiny_secuTrialR")
...
devtools::spell_check("shiny_secuTrialR", ignore = ignore_words)
```
...
library(lintr)
lint_package("shiny_secuTrialR", linters = with_defaults(camel_case_linter = NULL,
...
```{r, eval = FALSE}
library(rmarkdown)
render("vignettes/shiny_secuTrialR-package-vignette.Rmd", output_format=c("pdf_document"))
```
...
Requests for new features and bug fixes should first be documented as an [Issue](https://github.com/SwissClinicalTrialOrganisation/shiny_secuTrialR/issues) on GitHub.
